...
impl HistoryStore {
    ...
    pub fn add(
        ...
        self.entries.retain(|h| h.url != url);
    ...
    fn serialize_entries(entries: &[HistoryEntry]) -> String {
        ...
        for (i, e) in entries.iter().enumerate() {
            ...
            out.push_str(&format!("    \"played_at\": \"{}\"\n", Self::escape_json(&e.played_at)));
            out.push_str("  }");
            if i < entries.len() - 1 {
                out.push(',');
            }
    ...
    fn parse_entries(json: &str) -> Option<Vec<HistoryEntry>> {
    ...
    fn parse_one_entry(obj: &str) -> Option<HistoryEntry> {
        ...
        let played_at = Self::extract_string_field(obj, "played_at").unwrap_or_default();
        Some(HistoryEntry { name, url, genre, country, bitrate, played_at })
    }
